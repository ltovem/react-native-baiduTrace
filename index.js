...
export default class BaiduTrace {
    ...
    /**
     ...
     @param tag 请求标志
     */
    static boundSearchEntity(latitude,longitude,latitude2,longitude2,serviceID,tag,entityNames,activeTime,fieldName,inputCoordType = BTKCoordType.BTK_COORDTYPE_GCJ02,sortby = 1,outputCoordType = BTKCoordType.BTK_COORDTYPE_GCJ02,pageIndex = 1,pageSize = 10){
        RNBaiduTrace.boundSearchEntity(latitude,longitude,latitude2,longitude2,inputCoordType,entityNames,activeTime,fieldName,sortby,outputCoordType,pageIndex,pageSize,serviceID,tag)
    }
    /**
     周边搜索
    ...
    /**
     ...
     @param tag 请求标志
     */
    static queryLocalFenceStatus(monitoredObject,fenceIDs,tag){
        RNBaiduTrace.queryLocalFenceStatus(monitoredObject,fenceIDs,tag);
    }
    /**
     可以假设被监控对象处于某自定义的位置坐标时，其和地理围栏的位置关系。
     @param monitoredObject 围栏的监控对象名称
     ...
     @param tag 请求标志
     */
    static queryLocalFenceStatusByCustomLocation(monitoredObject,latitude,longitude,coordType,fenceIDs,tag){
        RNBaiduTrace.queryLocalFenceStatusByCustomLocation(monitoredObject,latitude,longitude,coordType,fenceIDs,tag);
    }
    /**
...
//++++++++++==============================================================
    ...
    /**
     ...
     @param {Function} cb = (Object）=> {{"response":data}
     */
    static onEntityAroundSearch(callback){
        listeners[callback] = DeviceEventEmitter.addListener(
            onEntityAroundSearch, result => {
                callback(result)
            })
    }
    /**
